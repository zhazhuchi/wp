...
(function() {
	...
	Calendar.prototype = {
		...
		getThemeHtml: function(dataObj) {
			...
			var ajaxyear = curr.trim().split("-")[0];
			var ajaxmonth = curr.trim().split("-")[1];
			...
			$.ajax({
				url: businessServerUrl + '/WaterDutyDeal/WaterDutyCalList.ashx',
				type: 'post',
				data: requestData,
				dataType: 'json',
				async: false,
			...
			for(var i = 0, len = dataObj.tmpInfo.length; i < len; i++) {
				...
				if(!templData) {
					...
					// 上一月和下一月不可点击模板------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
					var finds = false;
					for(var index = 0 ; index < needXRData.length ; index++){
						var tmpyear = dataObj.tmpInfo[i].date.trim().split("-")[0];
						var tmpmouth = dataObj.tmpInfo[i].date.trim().split("-")[1];

						if((ajaxyear == tmpyear) && (tmpmouth == ajaxmonth) && (needXRData[index].day == dataObj.tmpInfo[i].day)){
							if(needXRData[index].Status == "已完成"){
								template = '<div class="em-calendar-active-yiwancheng em-calendar-item isforbid{{isforbid}}" date="{{date}}"><span style="color:#FFF" class="day">{{day}}</span><p class="lunar">{{lunar}}</p></div>';
								html += Mustache.render(template, value);
								finds = true;
								break;
							} else if(needXRData[index].Status == "待办"){
								template = '<div class="em-calendar-active-daiban em-calendar-item isforbid{{isforbid}}" date="{{date}}"><span style="color:#FFF" class="day">{{day}}</span><p class="lunar">{{lunar}}</p></div>';
								html += Mustache.render(template, value);
								finds = true;
								break;
							} else if(needXRData[index].Status == "异常"){
								template = '<div class="em-calendar-active-yichang em-calendar-item isforbid{{isforbid}}" date="{{date}}"><span style="color:#FFF" class="day">{{day}}</span><p class="lunar">{{lunar}}</p></div>';
								html += Mustache.render(template, value);
								finds = true;
								break;
							} else {
